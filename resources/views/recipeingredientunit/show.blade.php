<x-app-layout>
    <x-slot name="header">
        ...
        <h2 class="py-2.5 px-5 bg-white font-semibold text-xl text-gray-800 leading-tight float-left"></h2>

        <form method="POST" action="{{ route('recipeingredientunit.destroy', ['id' => $recipeingredientunit->id ]) }}" class="float-right ml-3">
            {{ csrf_field() }}
            {{ method_field('DELETE') }}
        ...
        <a href="{{ route('recipeingredientunit.edit', ['id' => $recipeingredientunit->id ]) }}" class="float-right py-2.5 px-5 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200">Edit</a>
